...
class Mapper:
    ...
    def update(self, in_bounds_point_cloud: list = None, 
        ...
        if camera_images is not None and lidar_detections is not None:
            self.fixture_detector.map_fixtures(camera_images, self.robot_position, a)
    ...
    def visualize_grid(self):
        ...
        # Visualizar paredes
        plt.imshow(self.pixel_grid.arrays['walls'], cmap='gray', origin='lower', alpha=0.7)
        #plt.imshow(self.pixel_grid.arrays['victims'], cmap='gray', origin='lower', alpha=0.7)

        victims_array = self.pixel_grid.arrays['robot_detected_fixture_from']
        victims_coordinates = np.argwhere(victims_array)
        plt.scatter(victims_coordinates[:, 1], victims_coordinates[:, 0], color='red', marker='o', label='Victimas')

        victims_array2 = self.pixel_grid.arrays['victims']
        victims_coordinates2 = np.argwhere(victims_array2)
        print(victims_coordinates2)
        plt.scatter(victims_coordinates2[:, 1], victims_coordinates2[:, 0], color='blue', marker='x', label='Victimas2')

        plt.title('Visualizacion de Paredes en la Grilla y Victimas')
        plt.xlabel('Columnas')
        plt.ylabel('Filas')
